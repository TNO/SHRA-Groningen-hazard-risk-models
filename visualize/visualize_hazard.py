...
from visualize.visualization_tools import plot_hazard_maps
...
def main(args):
    ...
    if "lt_median_choice" in hazard.dims:
        lt_selection = config.get(
            "lt_median_choice", hazard["lt_median_choice"].values[-1]
        )
        hazard = hazard.sel(lt_median_choice=lt_selection)

    plot_hazard_maps(hazard, zones, fig_path=fig_path, show_max=True)
...
if __name__ == "__main__":
    ...
    args = (
        sys.argv[:]
        if sys.argv[1:]
        else ["dummy", "test_vis.yml", "--task", "test_vis_hazard"]
    )
    main(args)
    time1 = time.time()
